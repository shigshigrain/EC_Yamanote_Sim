...
/** 出力データファイル **/
FILE* fp7;				//停車時刻変更分・変電所出力総電力両出力用ファイル
...
#define dtime_pattern 5

double dtime1_dif[dtime_pattern] = { -5.0, -2.5, 0.0, 2.5, 5.0 };	//停車時間-5～+5秒で許容
double dtime2_dif[dtime_pattern] = { -5.0, -2.5, 0.0, 2.5, 5.0 };	//上に〃
double dtime3_dif[dtime_pattern] = { -5.0, -2.5, 0.0, 2.5, 5.0 };	//上に〃
double dtime4_dif[dtime_pattern] = { -5.0, -2.5, 0.0, 2.5, 5.0 };	//上に〃
double dtime5_dif[dtime_pattern] = { -5.0, -2.5, 0.0, 2.5, 5.0 };	//上に〃

// string dtime1_fol1[dtime_pattern];
// string dtime2_fol2[dtime_pattern];
// string dtime3_fol3[dtime_pattern];
// string dtime4_fol4[dtime_pattern];
// string dtime5_fol5[dtime_pattern];
...
void Make_ESD(ESD** esd, INI_ESD* ini, int i)
{
	char str[16];

	*esd = (ESD*)malloc(sizeof(ESD));
	if (*esd == NULL)
	{
		//エラー処理
	}
	...
	//鉄道総研の試験パラメータを参考
	(*esd)->name_ESD = i;
	(*esd)->Cfc = 10.05 / 1000.0;						//FC静電容量[F]
	(*esd)->RL = 0.0348;								//リアクトル巻線抵抗[Ω]
	(*esd)->RFL = 0.0348;								//FL巻線抵抗[Ω]

	(*esd)->kp_v_ch = 100.0;						//電圧制御系比例ゲイン(充電時)
	(*esd)->kp_v_disch = 50.0;						//電圧制御系比例ゲイン(放電時)
	(*esd)->ki_v_ch = 0.0;							//電圧制御系積分ゲイン(充電時)
	(*esd)->ki_v_disch = 0.0;						//電圧制御系積分ゲイン(放電時)

	(*esd)->kp_v = 0.0;								//電圧制御系比例ゲイン
	(*esd)->ki_v = 0.0;								//電圧制御系比例ゲイン
	(*esd)->P_v = 0.0;								//比例器出力初期値
	(*esd)->I_v = 0.0;								//積分器出力初期値

	(*esd)->kp_i = 0.001;							//電流制御系比例ゲイン
	(*esd)->ki_i = 0.0;								//電流制御系積分ゲイン
	(*esd)->P_i = 0.0;								//比例器出力初期値
	(*esd)->I_i = 0.0;								//積分器出力初期値


	/******** Batteryモデル ********/
	(*esd)->Eesd_min_c = Eesd_cap_c * ini->SOC_min / 100.0;							//ESDエネルギー下限値[kWh]
	(*esd)->Eesd_max_c = Eesd_cap_c * ini->SOC_max / 100.0;							//ESDエネルギー上限値[kWh]
	(*esd)->Eesd_min = Eesd_cap_c * ini->SOC_min / 100.0 * Ratio_E;					//ESDエネルギー下限値[J]
	(*esd)->Eesd_max = Eesd_cap_c * ini->SOC_max / 100.0 * Ratio_E;					//ESDエネルギー上限値[J]
	(*esd)->Eesd_minlim0_c = Eesd_cap_c * ini->SOC_min / 100.0 + lim_esd_c;			//[kWh]　エネルギー下限初期しきい値
	(*esd)->Eesd_maxlim0_c = Eesd_cap_c * ini->SOC_max / 100.0 - lim_esd_c;			//[kWh]　エネルギー上限初期しきい値
	(*esd)->Eesd_minlim0 = (Eesd_cap_c * ini->SOC_min / 100.0 + lim_esd_c) * Ratio_E;	//[J]　エネルギー下限初期しきい値
	(*esd)->Eesd_maxlim0 = (Eesd_cap_c * ini->SOC_max / 100.0 - lim_esd_c) * Ratio_E;	//[J]　エネルギー上限初期しきい値
	/*******************************/

	(*esd)->V_charge = ini->V_charge;				//充電開始電圧
	(*esd)->V_discharge = ini->V_discharge;			//放電開始電圧

	(*esd)->iesd_ref = 0.0;							//直流側電流指令値
	(*esd)->v0_ref = Vss1;							//直流側電圧指令値
	(*esd)->vp_ref = Vss1;							//直流側電圧指令値
	(*esd)->a = 0.0;								//通流率(vesd/v0)


	/** 変数 **/
	(*esd)->x = ini->Xesd;							//ESD設置位置[m]
	(*esd)->vp = Vss1;								//パンタ点電圧[V]
	(*esd)->idc = 0.0;								//直流側電流[A]
	(*esd)->vc = Vss1;								//FC電圧[V]
	(*esd)->ic = 0.0;								//FC電流[A]
	(*esd)->ich = 0.0;								//チョッパ出力電流[A]
	(*esd)->v0 = Vss1;								//IGBTコレクタエミッタ間電圧[V]


	/******** Batteryモデル ********/
	(*esd)->SOC = ini->SOC_ini;													//[%]
	(*esd)->Eesd_c = ini->SOC_ini / 100.0 * Eesd_cap_c * ini->SOC_max / 100.0;			//ESDエネルギー[kWh]
	(*esd)->Eesd = ini->SOC_ini / 100.0 * Eesd_cap_c * ini->SOC_max / 100.0 * Ratio_E;	//ESDエネルギー[J]
	/*******************************/

	(*esd)->vesd = Vbat_mod_rated * NoS;								//ESD電圧[V]
	(*esd)->Pesd_charge_lim_c = 2000.0;									//パワーリミット2000kW
	(*esd)->Pesd_charge_lim = 2000.0 * 1000.0;
	(*esd)->Pesd_discharge_lim_c = 1000.0;									//パワーリミット2000kW
	(*esd)->Pesd_discharge_lim = 1000.0 * 1000.0;
	(*esd)->Iesd_max = 2000.0 * 1000.0 / (*esd)->vesd;
	//	(*esd)->Iesd_max = 4000.0;
	//	(*esd)->Imax = 430.0;

	(*esd)->iesd = 0.0;							//ESD電流[A]
	(*esd)->Pesd = 0.0;							//ESDパワー[W]
	(*esd)->Pesd_c = 0.0;						//ESDパワー[kW]
	(*esd)->Pesd_loss = 0.0;					//内部抵抗でのロス[W]
	(*esd)->Pesd_loss_c = 0.0;					//内部抵抗でのロス[kW]
	(*esd)->Eesd_loss = 0.0;					//内部抵抗でのロス[J]
	(*esd)->Eesd_loss_c = 0.0;					//内部抵抗でのロス[kWh]

	(*esd)->Eesd_charge_c = 0.0;				//ESD充電エネルギー[kWh]
	(*esd)->Eesd_charge = 0.0;					//ESD充電エネルギー[J]
	(*esd)->Eesd_discharge_c = 0.0;				//ESD放電エネルギー[kWh]
	(*esd)->Eesd_discharge = 0.0;				//ESD放電エネルギー[J]
...
void Make_INV(INV** inv, INI_INV* ini, int i)
{
	char str[16];
	...
	*inv = (INV*)malloc(sizeof(INV));
	...
	(*inv)->name_INV = i;
	(*inv)->Cfc = 15.0 / 1000.0;						//FC静電容量[F]
	(*inv)->RL = 0.0348;								//リアクトル巻線抵抗[Ω]
	(*inv)->RFL = 0.0348;								//FL巻線抵抗[Ω]

	(*inv)->kp_v = 120.0 * 2.0;							//電圧制御系比例ゲイン
	(*inv)->ki_v = 0.0;								//電圧制御系積分ゲイン
	(*inv)->P_v = 0.0;								//比例器出力初期値
	(*inv)->I_v = 0.0;								//積分器出力初期値

	(*inv)->kp_i = 0.0;								//電流制御系比例ゲイン
	(*inv)->ki_i = 0.0;								//電流制御系積分ゲイン
	(*inv)->P_i = 0.0;								//比例器出力初期値
	(*inv)->I_i = 0.0;								//積分器出力初期値

	(*inv)->Vp_start = ini->Vp_start;				//[V]　動作開始電圧
	(*inv)->Vp_fin = ini->Vp_fin;				//[V]　動作終了電圧
	(*inv)->iinv_ref = 0.0;							//直流側電流指令値
	(*inv)->vp_ref = ini->Vp_start;					//直流側電圧指令値

	/** 変数 **/
	(*inv)->x = ini->Xinv;							//回生インバータ設置位置[m]
	(*inv)->vp = Vss1;								//パンタ点電圧[V]
	(*inv)->idc = 0.0;								//直流側電流[A]
	(*inv)->vc = Vss1;								//FC電圧[V]
	(*inv)->ic = 0.0;								//FC電流[A]
	(*inv)->iinv = 0.0;								//インバータ出力電流[A]

	(*inv)->Einv_c = 0.0;						//回生INV 回生エネルギー[kWh]
	(*inv)->Einv = 0.0 * Ratio_E;					//回生INV 回生エネルギー[J]
	/*******************************/

	(*inv)->Pinv_lim_c = 1000.0;
	(*inv)->Pinv_lim = 1000.0 * 1000.0;
	(*inv)->Iinv_max = 1500.0;					//回生INV 最大電流
	(*inv)->Iinv_rated = 617.0;					//回生INV 定格電流

	(*inv)->Pinv = 0.0;							//回生INV 回生パワー[W]
	(*inv)->Pinv_c = 0.0;						//回生INV 回生パワー[kW]
}
...
void Run_pattern(TRAIN* tra, STATION* sta, double t)
{
	int i;
	...
	if (tra->accelflag == 4)		/*停車中の動作を決定*/
	{
		...
		else if (t < tra->T_delay)
		{
			...
			if (sta[2].Xs - 200.0 <= tra->x && tra->x < sta[2].Xs + 200.0) {	//目白
				...
				if (tra->Type == 0) {
					if (tra->t_stop - tra->t_stop_old >= (stoptime + dtime1_dif[dtime_pattern])) tra->accelflag = 1;
				}
				...
				// if (tra->t_stop - tra->t_stop_old >= stoptime) tra->accelflag = 1;
				if (tra->Type == 0) {
					if (tra->t_stop - tra->t_stop_old >= (stoptime - dtime1_dif[dtime_pattern] + dtime2_dif[dtime_pattern])) tra->accelflag = 1;
				}
				...
				// if (tra->t_stop - tra->t_stop_old >= stoptime) tra->accelflag = 1;
				if (tra->Type == 0) {
					if (tra->t_stop - tra->t_stop_old >= (stoptime - dtime2_dif[dtime_pattern] + dtime3_dif[dtime_pattern])) tra->accelflag = 1;
				}
				...
				// if (tra->t_stop - tra->t_stop_old >= stoptime) tra->accelflag = 1;
				if (tra->Type == 0) {
					if (tra->t_stop - tra->t_stop_old >= (stoptime + dtime4_dif[dtime_pattern])) tra->accelflag = 1;
				}
				...
				// if (tra->t_stop - tra->t_stop_old >= stoptime) tra->accelflag = 1;
				if (tra->Type == 0) {
					if (tra->t_stop - tra->t_stop_old >= (stoptime - dtime4_dif[dtime_pattern] + dtime5_dif[dtime_pattern])) tra->accelflag = 1;
				}
...
int main(void)
{
	...
	double sub_output[dtime_pattern];
	...
	while (sim_endflag == 0) {
		...
		Make_file7(&fp7);
		fprintf(fp7, "目白発車時刻変化分[sec],高田馬場[sec],新大久保[sec],代々木[sec],原宿[sec],変電所出力総電力量[kWh]\n");
		...
		for (i = 0; i < NUM_sub; i++)
		{
			sub_output[i] += sub[i]->Ess_c;
		}

		if (Consider_loop_flag == 0) {
			fprintf(fp7, "%s, %s, %.1f, %f, %f, %f, %f, %f, %f\n", "", "", dtime1_dif[dtime_pattern], dtime2_dif[dtime_pattern], dtime3_dif[dtime_pattern], dtime4_dif[dtime_pattern], dtime5_dif[dtime_pattern], sub_output);
		}
		...
		if (dtime_state1 == (dtime_pattern - 1) && dtime_state2 == (dtime_pattern - 1) && dtime_state3 == (dtime_pattern - 1) && dtime_state4 == (dtime_pattern - 1) && dtime_state5 == (dtime_pattern - 1)) { sim_endflag++; break; }

		if (Consider_loop_flag == 1) break;

		if (dtime_state5 == (dtime_pattern - 1)) {
			dtime_state5 = 0;
			if (dtime_state4 == (dtime_pattern - 1)) {
				dtime_state4 = 0;
				if (dtime_state3 == (dtime_pattern - 1)) {
					dtime_state3 = 0;
					if (dtime_state2 == (dtime_pattern - 1)) {
						dtime_state2 = 0;
						dtime_state1++;
					}
					else {
